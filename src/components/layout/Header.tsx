    ...
    return (
        <div className="bg-bg-header w-full xl:pl-[123px] lg:px-[96px] px-[10px]  pt-[33px] pb-[22px] flex justify-between items-center relative">
            ...
            <div className="flex lg:gap-[72px] gap-[20px] items-center">
                ...
                <Button className="text-white max-lg:hidden rounded-[9px] bg-bg-button px-6 py-3 text-[20px] font-medium">
                    {location === "/" ? "Buy Now" : "Connect Wallet"}
                </Button>
                <div className="dropdown lg:hidden">
                    <DropdownMenu>
                        <DropdownMenuTrigger><Grip className="w-[42px] h-[44px] text-white" /></DropdownMenuTrigger>
                        <DropdownMenuContent className="bg-[#8B7533] mr-5">
                            <DropdownMenuItem>
                                <Link to="/" className={`py-2 text-[27px] font-bold ${location === "/" ? "text-bg-active" : "text-white"}`}>Home</Link>
                            </DropdownMenuItem>
                            <DropdownMenuItem>
                                <Link to="/memebot" className={`text-[27px] py-2 font-bold ${location === "/memebot" ? "text-bg-active" : "text-white"}`}>$MEMES bot</Link>
                            </DropdownMenuItem>
